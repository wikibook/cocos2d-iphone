...
- (void)ccTouchEnded:(UITouch *)touch withEvent:(UIEvent *)event
{
    ...
    if (abs(diff.x) > abs(diff.y)) {
        if (diff.x > 0) {
            playerPos.x += tmap.tileSize.width;
        ...
        if (diff.y > 0) {
            playerPos.y += tmap.tileSize.height;
        } else {
            playerPos.y -= tmap.tileSize.height;
        }
    }
    
    if (playerPos.x <= (tmap.mapSize.width * tmap.tileSize.width) &&
        playerPos.y <= (tmap.mapSize.height * tmap.tileSize.height) &&
        playerPos.y >= 0 &&
        playerPos.x >= 0 )
    {
        // 드래곤의 새로운 위치 지정
